	...
	init: function () {
		...
		for (var slug in Emic.db) {
			...
			obj.onclick = function () {
				...
				Emic.textarea.focus();
				// Optionally hide emoji picker after selection:
				Emic.container.classList.remove('active');
			};
		...
		Emic.emic_btn.onclick = function (e) {
			e.stopPropagation();
			Emic.container.classList.toggle('active');
		};

		// Optional: Hide picker if clicked outside
		document.addEventListener('click', function (e) {
			if (!Emic.emic_btn.contains(e.target) && !Emic.container.contains(e.target)) {
				Emic.container.classList.remove('active');
			}
		});
	}
