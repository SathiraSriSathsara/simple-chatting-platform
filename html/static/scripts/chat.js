...
var WAFullScreenChat = {
  ...
  scroll: function () {
    setTimeout(function () {
      WAFullScreenChat.chat_box.scrollTop =
        WAFullScreenChat.chat_box.scrollHeight;
    }, 0);
  },
  ...
  notif: {
    ...
    create: function (from, message) {
      if (WAFullScreenChat.is_focused || !WAFullScreenChat.notif.enabled)
        return;
      WAFullScreenChat.notif.msgs++;
      WAFullScreenChat.notif.favicon("blue");
      document.title =
        "(" + WAFullScreenChat.notif.msgs + ") " + WAFullScreenChat.new_title;
      ...
      message = message.text?.replace(/(<([^>]+)>)/gi, "");
    ...
    clear: function () {
      typeof WAFullScreenChat.notif.active === "undefined" ||
        WAFullScreenChat.notif.active.close();
    },
  ...
  send_event: function () {
    ...
    if (value == "") return;
    WAFullScreenChat.send_msg(value); // Send just the string!
  ...
  typing: {
    ...
    create: function (nick) {
      ...
      body.innerHTML =
        '<span class="one">&bull;</span><span class="two">&bull;</span><span class="three">&bull;</span>';
    ...
    update: function () {
      if (
        WAFullScreenChat.is_typing &&
        WAFullScreenChat.textarea.value === ""
      ) {
        WAFullScreenChat.socket.emit(
          "typing",
          (WAFullScreenChat.is_typing = false)
        );
      }
      if (
        !WAFullScreenChat.is_typing &&
        ...
        WAFullScreenChat.socket.emit(
          "typing",
          (WAFullScreenChat.is_typing = true)
        );
      }
    },
  },
  ...
  force_login: async function (fail) {
    if (typeof fail !== "undefined") {
      alert(fail);
    }

    // Check localStorage for existing nickname
    let nick = localStorage.nick;
    if (nick && nick.trim()) {
      sessionStorage.nick = nick; // sync to session
      WAFullScreenChat.socket.emit("login", { nick: nick });
      return;
    }

    // If not in localStorage, try fetching from API (optional userId can be used here)
    const userId = localStorage.getItem("user_id"); // adjust as needed
    if (userId) {
      const fetchedNick = await fetchNickname(userId);
      if (fetchedNick) {
        localStorage.nick = fetchedNick;
        sessionStorage.nick = fetchedNick;
        WAFullScreenChat.socket.emit("login", { nick: fetchedNick });
        return;
      }
    }

    // If no nickname found, prompt the user
    nick = prompt("Your nick:", sessionStorage.nick || "").trim();
  ...
  reload: function () {
    const nick = sessionStorage.nick || localStorage.nick;
    if (nick) {
  ...
  init: function (socket) {
    ...
    window.addEventListener("focus", function () {
      ...
      typeof WAFullScreenChat.notif.ttout === "undefined" ||
        clearInterval(WAFullScreenChat.notif.ttout);
    ...
    WAFullScreenChat.socket.on(
      "previous-msg",
      WAFullScreenChat.user.previous_messages
    );
